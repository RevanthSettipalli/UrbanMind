...
risk_score = min(100, round((df['temperature'].mean() * 2) + (df['humidity'].mean() * 0.5), 1))
operational_health = max(70, round(100 - risk_score * 0.3, 1))
forecast_confidence = 97
national_score = round(100 - risk_score * 0.2, 1)

ex1, ex2, ex3, ex4 = st.columns(4)

ex1.metric("Operational Health", f"{operational_health}%")
ex2.metric("Forecast Confidence", f"{forecast_confidence}%")
ex3.metric("National Risk", f"{risk_score}/100")
ex4.metric("National Score", national_score)
...
st.subheader("🏆 Board-Level KPI Dashboard")

k1, k2, k3, k4 = st.columns(4)

k1.metric("Best City", best_city)
k2.metric("Priority City", worst_city)
k3.metric("Records Analysed", len(df))
k4.metric("Cities Monitored", report['city'].nunique() if 'city' in report.columns else 0)
...
st.subheader("🤖 Board-Level Recommendations")

st.info("Increase monitoring in high-risk regions and maintain forecast readiness.")
st.info("Expand green infrastructure and sustainability programs.")
st.info("Prepare resource allocation plans for extreme weather events.")
st.info("Continue AI-driven city intelligence and predictive monitoring.")
...
st.subheader("🏛 Master Intelligence Package")
...
forecast_data = {
    "next_day_temp": round(df["temperature"].mean(), 1),
    "confidence": accuracy,
    "accuracy": accuracy,
    "risk": "Moderate",
